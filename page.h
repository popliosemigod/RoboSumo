...
/* ---------- bancada / diagnostico ---------- */
.diag{display:grid;grid-template-columns:repeat(auto-fit,minmax(268px,1fr));gap:6px}
.dg{display:flex;align-items:center;gap:9px;padding:9px 11px;border-radius:9px;
    background:var(--panel2);border:1px solid var(--line);font-size:12px}
.dg .dot{width:9px;height:9px;border-radius:50%;flex:none;background:var(--dim)}
.dg.ok  .dot{background:var(--acc);box-shadow:0 0 8px var(--acc)}
.dg.bad .dot{background:var(--hot);box-shadow:0 0 8px var(--hot)}
.dg.ok{border-color:rgba(0,229,160,.28)}
.dg.bad{border-color:rgba(255,77,109,.35)}
.dg b{flex:1;font-weight:700;letter-spacing:.03em}
.dg i{font-style:normal;color:var(--dim);font-size:11px;text-align:right}
...
function dgRow(cls,nome,det){
 return '<div class="dg '+cls+'"><span class="dot"></span><b>'+nome+'</b><i>'+det+'</i></div>';}

/* GPIO34/35 nao tem pull-up interno: pino solto le BAIXO e parece borda
   para sempre. O AO solto encosta no fundo de escala ou no zero. Nenhum dos
   dois PROVA fiacao - por isso so ha verde quando o valor cai numa faixa
   que um modulo IR de verdade produziria. */
function irCls(a){return (a>60&&a<4030)?"ok":"bad";}
function irTxt(a,d){
 const s=(d?"DO alto":"DO baixo")+" &middot; AO "+a;
 if(a>=4030)return s+" &middot; fundo de escala (solto?)";
 if(a<=60)  return s+" &middot; colado no zero (solto?)";
 return s;}

function diag(){
 $("diag").innerHTML="<i style='color:var(--dim)'>varrendo os dois barramentos...</i>";
 fetch("/api/selftest").then(r=>r.json()).then(d=>{
  const bo=d.busOled.map(hex).join(" ")||"vazio", bt=d.busTof.map(hex).join(" ")||"vazio";
  let h="";
  h+=dgRow(d.hz>150?"ok":"bad","ESP32 / malha de controle",d.hz+" Hz &middot; heap "+((d.heap/1024)|0)+" kB");
  h+=dgRow("ok","Wi-Fi / painel",d.ap+" aparelho(s) no AP");
  h+=dgRow(d.oled?"ok":"bad","OLED SSD1306 (SDA21/SCL22)",
     d.oled?hex(d.oledAddr)+" respondendo":"barramento: "+bo);
  h+=dgRow(d.tofL?"ok":"bad","Olho ESQ VL53L0X (16/17)",d.tofL?"medindo em 0x30":"sem resposta");
  h+=dgRow(d.tofR?"ok":"bad","Olho DIR VL53L0X (16/17)",d.tofR?"medindo em 0x29":"sem resposta");
  ...
  $("diag").innerHTML=h;
  $("diagAt").textContent=fmt(d.at);
 }).catch(()=>{$("diag").innerHTML=
   "<i style='color:var(--hot)'>a placa nao respondeu ao diagnostico</i>";});}
...
<div class="grid">
  ...
  <!-- ============ BANCADA ============ -->
  <div class="card wide">
    <h2>Bancada &mdash; o que respondeu de verdade</h2>
    <div class="diag" id="diag"><i style="color:var(--dim)">varrendo...</i></div>
    <div class="row" style="margin-top:10px">
      <button onclick="diag()">&#8635; REVARRER BARRAMENTOS</button>
      <button onclick="cmd('testoled')">&#9635; TESTAR DISPLAY</button>
      <button onclick="cmd('beep')">&#9834; TESTAR BUZZER</button>
      <button onclick="cmd('reboot')">&#9211; REINICIAR A PLACA</button>
    </div>
    <div class="note">Cada linha mostra o que a placa <b>achou agora</b>, nao o que devia
      existir. Verde = respondeu; vermelho = nao respondeu; cinza = o pino sozinho nao
      prova nada. Ultima varredura com <span id="diagAt">--</span> de placa ligada.</div>
  </div>
  ...
  <div class="card">
    ...
    <div class="kv"><span>Olho esquerdo (ToF)</span><b id="dl">--</b></div>
    <div class="bar" style="margin-bottom:6px"><i id="bl"></i></div>
    <div class="kv"><span>Olho direito (ToF)</span><b id="dr">--</b></div>
  ...
  <div class="card">
    ...
    <div class="row" style="margin-top:10px">
      <button onclick="cmd('irEscuro')">1. CAPTURAR ESCURO</button>
      <button onclick="cmd('irClaro')">2. CAPTURAR CLARO</button>
      <button onclick="cmd('irLimpa')">zerar</button>
    </div>
    <div class="kv" style="margin-top:8px"><span>Capturado no escuro (esq / dir)</span><b id="cesc">--</b></div>
    <div class="kv"><span>Capturado no claro (esq / dir)</span><b id="ccla">--</b></div>
    <div class="kv"><span>Separacao entre os dois</span><b id="cmar">--</b></div>
    <div class="note">Ponha o robo na zona escura e aperte <b>1</b>; ponha em cima da faixa
      clara e aperte <b>2</b>. O limiar e o sentido do sinal saem sozinhos da medida &mdash;
      inclusive se o seu modulo for invertido. Separacao abaixo de ~250 e pouco: aproxime
      o sensor do chao.</div>
...
function poll(){
 fetch("/api/state").then(r=>r.json()).then(s=>{
  ...
  $("cesc").textContent=s.cEsc?s.cEscL+" / "+s.cEscR:"nao capturado";
  $("ccla").textContent=s.cCla?s.cClaL+" / "+s.cClaR:"nao capturado";
  $("cmar").textContent=(s.cEsc&&s.cCla)?s.cMar+(s.cMar<250?" (pouco!)":" (ok)"):"--";
